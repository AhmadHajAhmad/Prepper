...
router.get('/',authenticate, controller.getAllSuppliesByUser); //2 - verified

//3-Get a supply doc by supplyid & userid
router.get('/:supplyid',authenticate, controller.getByUserId);//3 - verified

//4- Create Supply
router.post('/',authenticate, controller.createSupply);//4 - verified


//5-Delete a supply by itsId & UserId
router.delete('/:supplyid',authenticate, controller.deleteByUserId);//5-done - verified
